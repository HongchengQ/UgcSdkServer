...
public class ConversionService {
    ...
    public Object processForwardConversion(String outputFormat, ConversionData requestData) {
        try {
            ...
            // 执行正向转换
            Object result = processForwardConversionLogic(fileData, outputFormat);
     ...
     */
    public String processReverseConversion(GiFileType targetFileType, ConversionData requestData) {
        try {
            // 解码base64数据
            byte[] fileData = Base64.getDecoder().decode(requestData.getBase64Data());

            // 执行反向转换
            byte[] binaryData = processReverseConversionLogic(fileData, targetFileType, requestData.getFileType());
     ...
     * 正向转换核心逻辑
     */
    private Object processForwardConversionLogic(byte[] fileData, String outputFormat) {
        try {
            // 解码
     ...
     * 反向转换核心逻辑
     */
    private byte[] processReverseConversionLogic(byte[] fileData, GiFileType targetFileType, String originalFileTypeStr) {
        try {
            // 根据用户选择的目标文件类型进行转换
            return encodeService.encodeGiFile(fileData, targetFileType, originalFileTypeStr);
        } catch (Exception e) {
